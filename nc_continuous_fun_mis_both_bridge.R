...
qfunc <- function(tau, data){
  q_est <- with(data, c(1 + exp((1 - 2 * A) * (cbind(1, A, Z) %*% tau))))
  return(q_est)
}

hfunc <- function(eta, data){
  h_est <- with(data, c(exp(cbind(1, A, W) %*% eta)))
  return(h_est)
}
...
U_DR <- function(par, data) {
  ...
  h1 <- with(data, c(exp(cbind(1, 1, W) %*% eta)))
  h0 <- with(data, c(exp(cbind(1, 0, W) %*% eta)))
  U_q <- with(data,
              c((1 - Y) * q_est) * cbind(1, A, W, X, A * W, A * X, W * X, A * W * X) -
                c(1 - Y) * cbind(2, 1, 2 * W, 2 * X, W, X, 2 * W * X, W * X))
  U_h <- with(data,
...
nc_cont_dr <- function(data, beta_init = -1, qinit = rep(0, 3), hinit = rep(-0.5, 3)) {
...
nc_cont_ipw <- function(data) {
  opt <- nlm(f = GMMF, p = rep(0, 4), mrf = U_IPW, data = data, iterlim = 200)
...
nc_cont_or <- function(data, init = rep(-0.5, 4)) {
