...
options(warn = 2)
param_grid <- expand.grid(N = c(3500, 5000),
                          beta0 = log(0.2))
...
sim_once <- function(N, beta0) {
  data <- DGP_bin(N, beta0)
  ...
  ipw_results <- try(nc_bin_ipw(data), silent = T)
  dr_results <- try(nc_bin_dr(data), silent = T)
  or_results <- try(nc_bin_or(data), silent = T)
...
result <- mclapply(1:NSIM, 
                   function(ii) {
                     print(ii) 
                     set.seed(ii)
                     sim_once(N = N, beta0 = beta0)
                   }, 
                   mc.cores = n.cores)

saveRDS(result, 
        file = paste0("results/nc_bin_type_N_", N, 
