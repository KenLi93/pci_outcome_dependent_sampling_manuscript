library(dplyr)
library(xtable)
param_grid <- data.frame(N = c(3500, 5000),
                         beta0 = log(0.2))
...
for (ii in 1:nrow(param_grid)) {
  ...
  result <- readRDS(paste0("results/nc_bin_type_N_", N, 
                           "_beta0_", round(beta0 * 100), ".rds")) %>% 
    bind_rows %>% as.data.frame %>% filter(DR_se <= 3)
...
saveRDS(param_grid, file = "bin_sim_results.rds")
print(result_grid_xtable, include.rownames = F,
      file = "bin_sim_xtable.txt")
...
print(result_grid_xtable2, include.rownames = F, file = "complete_bin_xtable_up.txt")
print(result_grid_xtable3, include.rownames = F, file = "complete_bin_xtable_down.txt")
